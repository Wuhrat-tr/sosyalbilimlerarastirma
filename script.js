...
* Araştırmanın sınanmasını zorlaşması ile yansızlığı arttırır.
...
*
  ...
  * Doğru hipotez yoktur (hipotezler nihai olarak ispatlanmaz, sadece doğrulanır veya yanlışlanır)

* Kuramsal bir temele bağlı olmalıdır.
* Ara değişken varsa net görünmelidir.
  → yani tüm değişkenler görülmeli.

* bilinenlerle zıt şeyler yazamazsın.
...
* Açık basit ve işlevsel şekilde ifade edilmelidir.
  * → 
  * → sınanabilir, ölçülebilir olması.
...
> → istemelerine karşılık vermemek mümkün oldukça azaltılacak.`,
...
* Bütün mesele ölçebilmek | Araştırma Tasarımı nirengi noktası noktası 
...
| **Pardöse hikayesi** | ölçebilmek önemli<br>ısı pompası olmayan araçları<br>sattılar var diye<br>Elektrikli Araç <br>ölçmeyi bilmezsen olmaz<br>kamu araç tarex |
...
| | |
| :--- | :--- |
| Önce betimleyin<br>ne olduğunu açıklamaya çalışınız.<br>Sonra değişkenlerini ortaya koyunuz. | enflasyon<br>faiz arasında<br>değişken var mı<br>hangisi etken değişken<br>hangisi edilgen değişken |

→ değişen aslında o olgunun kendisi değildir. sahip oldukları değişir.
...
destek
(Bağımsız → Cinsiyet, Bağımlı → destek)
...
→ burada bağımlı değişken doğum oranlarının düşüşüdür, bağımsız değişken kadınların iş yerindeki etkisi.
ülkende umut görürsen çocuk yaparsın.

Bağımsız değişkenden etkilenen bağımlı değişken denir.

bağımlı değişken ile bağımsız değişken arasındaki yol ve bu değişkenler aracılığıyla işlediği değişkendir.
...
→ Evlilikte toplumsal bütünleşme daha fazladır.

---

| | |
| :--- | :--- |
| örneğin bir suça yönelik davranış, buna neden olan 4 b.d.<br>1-) ekonomik darlık,<br>2-) Gelecekte suç işleme fırsatı,<br>3-) Sapkın geçmişi varsa<br>4-) Cezaların caydırıcı olmaması,<br><br>→ bağımsız<br>**Ailenin dağılması / çocuk**<br>→ ara değişken<br>* çocuklukta özgüven azlığı<br>* depresyon<br>* Akademik başarısızlık<br>* iş bulma olasılığının düşüklüğü<br><br>→ Bağımlı<br>**Büyüyünce gelir düzeyi düşük olacak.** | 33 yaşında askere gitmiş. Orda doktor dışlanmış. Orduda sinemacı olmuş. hayattaki en büyük korkum şu: çocuğunun istemediği nahoş bir hayat sürmesi korkusu. çocuğun olduğu için hayattan kopmayı düşünemezsin. Bunu sorarsan buda Ara değişkendir şimdi.<br><br>ülkenin %89'unun 10.000 TL bankada bulunmuyor. Bankalardaki mevduatın %78'i %1'in elindeymiş. |

---

Yüzde 90 bağımlı, bağımsız değişken bahsederiz.
...
Nedensel ilişki olmalı bağımsız değişken ..

→ Doğru orantı varsa + korelasyon.
...
| - Yeniden oluşturulmuş mantık kullanır.<br>↓<br>**Tümden gelim**<br>metotta genel hatları vardır zaten | - uygulamada mantık kullanır.<br>↓<br>net bir soru, net bir şey yoktur.<br>**Kervan yolda düzülür** |
| - doğrusal araştırma yolunu izler.<br>↓<br>Ne yapacağı bellidir. → daha az esnektir. (B, C planı yaparız biz) | - doğrusal olmayan yolu izler.<br>↓<br>ne yapacağı belli değildir. → (Süpriz) |
